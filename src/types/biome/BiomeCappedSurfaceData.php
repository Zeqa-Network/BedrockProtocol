...
final class BiomeCappedSurfaceData{
	...
	public static function read(ByteBufferReader $in) : self{
		...
		for($i = 0, $count = VarInt::readUnsignedInt($in); $i < $count; ++$i){
			$floorBlocks[] = LE::readSignedInt($in);
		}

		$ceilingBlocks = [];
		for($i = 0, $count = VarInt::readUnsignedInt($in); $i < $count; ++$i){
			$ceilingBlocks[] = LE::readSignedInt($in);
		}

		$seaBlock = CommonTypes::readOptional($in, LE::readSignedInt(...));
		$foundationBlock = CommonTypes::readOptional($in, LE::readSignedInt(...));
		$beachBlock = CommonTypes::readOptional($in, LE::readSignedInt(...));
	...
	public function write(ByteBufferWriter $out) : void{
		...
		CommonTypes::writeOptional($out, $this->seaBlock, LE::writeSignedInt(...));
		CommonTypes::writeOptional($out, $this->foundationBlock, LE::writeSignedInt(...));
		CommonTypes::writeOptional($out, $this->beachBlock, LE::writeSignedInt(...));
	}
}
