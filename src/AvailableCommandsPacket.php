...
class AvailableCommandsPacket extends DataPacket implements ClientboundPacket{
	...
	protected function decodePayload(ByteBufferReader $in, int $protocolId) : void{
		...
		for($i = 0, $count = VarInt::readUnsignedInt($in); $i < $count; ++$i){
			$this->commandData[] = $this->getCommandData($enums, $postfixes, $chainedSubCommandData, $in, $protocolId);
		}
	 ...
	 * @throws DataDecodeException
	 */
	protected function getCommandData(array $enums, array $postfixes, array $allChainedSubCommandData, ByteBufferReader $in, int $protocolId) : CommandData{
		$name = CommonTypes::getString($in);
		...
		for($overloadIndex = 0, $overloadCount = VarInt::readUnsignedInt($in); $overloadIndex < $overloadCount; ++$overloadIndex){
			$parameters = [];
			$isChaining = $protocolId >= ProtocolInfo::PROTOCOL_1_20_10 && CommonTypes::getBool($in);
	 ...
	 */
	protected function putCommandData(CommandData $data, array $enumIndexes, array $softEnumIndexes, array $postfixIndexes, array $chainedSubCommandDataIndexes, ByteBufferWriter $out, int $protocolId) : void{
	...
	protected function encodePayload(ByteBufferWriter $out, int $protocolId) : void{
		...
		foreach($this->commandData as $data){
			$this->putCommandData($data, $enumIndexes, $softEnumIndexes, $postfixIndexes, $chainedSubCommandDataIndexes, $out, $protocolId);
		}
